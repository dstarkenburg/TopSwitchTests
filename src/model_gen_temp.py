...
with h5py.File(filename, "r") as f:
    for i in f["train_data"].keys():
        if i != "num_samples" and i != "index" and len(f["train_data"][i]["branch"]) == 2:
...
with h5py.File(filename, "r") as f:
    for i in f["test_data"].keys():
        if i != "num_samples" and i != "index" and len(f["test_data"][i]["branch"]) == 2:
...
with h5py.File(filename, "r") as f:
    for i in f["val_data"].keys():
        if i != "num_samples" and i != "index" and len(f["val_data"][i]["branch"]) == 2:
...
num_branches = len(y_train_temp[1])
input_dim = len(x_train_temp[1])
...
data = OpsDataset(x_train_temp, y_train_temp)
